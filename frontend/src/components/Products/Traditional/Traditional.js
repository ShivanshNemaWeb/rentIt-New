...
import {getTraditionalProducts} from '../../../actions/productActions';
...
const Traditional = () => {
  ...
  const [addCartSuccess,setAddCartSuccess] = useState(false);
    ...
    useEffect(() => {
      ...
      setTimeout(()=>{
        dispatch(getTraditionalProducts())
    },1000)
    },[dispatch]);
...
const handleDislikeClick =  (e,productId) => {
  ...
  // ... (existing code)
   ...
   <div className="row">
       {
   TraditionalProducts && TraditionalProducts.slice(0, 4).map((product)=>{
               return(<>
   ...
   <Link to="/Traditional"><h6><a href="">View More</a> <i className='fa fa-arrow-right circle rounded-circle p-1' style={{color:"white", backgroundColor:"rgba(0, 52, 82,1)"}}></i></h6></Link>
   </div>
</div>
</section>
    </>)
