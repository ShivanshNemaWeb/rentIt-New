...
import {getPartyProducts} from '../../../actions/productActions';
...
const handleDislikeClick =  (e,productId) => {
  ...
  dispatch(dislike(productId,history));
   ...
   <div className="row">
       {
   PartyProducts && PartyProducts.slice(0, 4).map((product)=>{
               return(<>
   ...
   <Link to="/Party"><h6><a href="">View More</a> <i className='fa fa-arrow-right circle rounded-circle p-1' style={{color:"white", backgroundColor:"rgba(0, 52, 82,1)"}}></i></h6></Link>
   </div>
</div>
</section>
    </>)
